...
- **Corte por família** (em `triagem.familias_condenadas`, aplicado por `mine.py check` antes de gastar crédito): família = primeira palavra de conteúdo da keyword ("calculadora de bitcoin lucro" → `bitcoin`). Uma família é condenada quando acumula 4+ rejeições **sem nenhuma aprovada**, ou 10+ checagens com menos de 15% de aprovação; as candidatas restantes dela viram `descartada` sem consultar a Serper. O agrupamento é grosseiro de propósito — o que protege as famílias boas é a exigência de zero aprovadas, não a precisão: medido em 15/09/2026, `gasta` (17 aprovadas em 24) sobrevive enquanto `gerador` (0 de 11) e `salario` (0 de 4) morrem. Créditos da Serper são finitos (2500 no total, ~1336 em 15/09/2026) e a mineração cega já queimou famílias inteiras de uma vez.
- `scripts/triagem.py` — o julgamento que regex não faz, via IA grátis: lê a SERP e responde (1) o buscador quer mesmo uma ferramenta, ou quer salário de mercado/cotação/vaga? (2) cada resultado do top-3 é ferramenta interativa ou só artigo explicando? (3) um AI Overview responderia a busca inteira sem dado do usuário? Só aprova quando as três dão certo. Cache em `backlog/triagem.json`, versionado por `VERSAO` (suba ao mudar critério e os vereditos velhos são refeitos).
  **Pitfall real, encontrado numa auditoria em 15/09/2026**: o critério (3) confundia "existe uma fórmula/proporção simples por trás" com "a IA responde sem dado do usuário" — reprovou `cloro-piscina`, `calculadora-tijolos`, `calculadora-telhas`, `tijolos-casa`, `galinhas-por-m2` (todas com input real de m²/litros do imóvel do usuário) achando que "regra fixa de X por Y" bastava pra IA responder, ignorando que sem a MEDIDA do usuário (área da obra, volume da piscina) não existe número nenhum. A distinção certa (já no prompt do `MOLDE`): "o número que falta pode ser QUALQUER coisa (imóvel/volume do usuário) → false" vs. "o número que falta tem faixa estreita e conhecida (potência típica de um aparelho específico) → true". Ao mexer no prompt de novo, preservar essa distinção — é fácil regredir pra "tem fórmula = respondida pela IA".
...
- Página de ferramenta = head (meta/SEO) + `<main>` (H1 + `.app` + `.ad-slot` + `<details class="conteudo">` + `<nav class="explicacao">` com os relacionados) + scripts `config.js`, `chrome.js` e o JS da ferramenta. O template cria só a parte de cima; `backfill_explicacao.py` acrescenta os relacionados (visíveis, em `<nav>` — nada de `<details>` ali) e `data-footer`, e `conteudo.py` acrescenta a seção de conteúdo dentro de um `<details class="conteudo">` **fechado por padrão** (decisão do dono em 17/09/2026: a calculadora é o produto, o texto só aparece para quem quiser). O HTML continua inteiro na página — o Google indexa conteúdo dentro de `<details>` —, mas se o AdSense voltar a acusar "low value content", abrir a seção (`<details open>`) é a primeira coisa a testar. Acrescenta também o FAQPage JSON-LD real. Nada disso é escrito à mão.
